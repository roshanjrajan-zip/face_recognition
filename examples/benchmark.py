...
def run_test(
    setup,  # pyright: ignore[reportMissingParameterType,reportUnknownParameterType]
    test,  # pyright: ignore[reportMissingParameterType,reportUnknownParameterType]
    iterations_per_test=5,  # pyright: ignore[reportMissingParameterType]
    tests_to_run=10,  # pyright: ignore[reportMissingParameterType]
):
    fastest_execution = min(
        timeit.Timer(
            test,  # pyright: ignore[reportUnknownArgumentType]
            setup=setup,  # pyright: ignore[reportUnknownArgumentType]
        ).repeat(
            tests_to_run,
            iterations_per_test,
        ),
