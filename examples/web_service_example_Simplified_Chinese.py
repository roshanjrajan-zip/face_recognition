...
def allowed_file(
    ...
    return (
        "." in filename
        and filename.rsplit(  # pyright: ignore[reportUnknownMemberType]
            ".",
            1,
        )[1].lower()
...
def detect_faces_in_image(  # pyright: ignore[reportUnknownParameterType]
    file_stream,  # pyright: ignore[reportMissingParameterType,reportUnknownParameterType]
):
    ...
    img = face_recognition.load_image_file(  # pyright: ignore[reportUnknownMemberType]
        file_stream,  # pyright: ignore[reportUnknownArgumentType]
    )
    # 为用户上传的图片中的人脸编码
    unknown_face_encodings = face_recognition.face_encodings(  # pyright: ignore[reportUnknownMemberType,reportUnknownVariableType]
        img,
    )
    ...
    if (
        len(
            unknown_face_encodings,  # pyright: ignore[reportUnknownArgumentType]
        )
        > 0
    ):
        face_found = True
    ...
    return jsonify(  # pyright: ignore[reportUnknownVariableType]
        result,
    )
